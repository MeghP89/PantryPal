...
interface FloatingImagePickerButtonProps {
  disabled?: boolean;
}

export default function FloatingImagePickerButton({ disabled = false }: FloatingImagePickerButtonProps): JSX.Element {
  ...
  const handlePress = () => {
    if (disabled) {
      alert('Limit reached. Clear items to make space.');
      return;
    }
    if (!expanded) {
  ...
  return (
    <View style={styles.absoluteContainer}>
      <Animated.View style={[styles.widgetContainer, widgetStyle]}>
        <Pressable
          style={[styles.widgetButton, disabled && styles.disabledButton]}
      ...
      <View style={styles.fabContainer}>
        <Pressable
          style={[styles.fabButton, disabled && styles.disabledButton]}
          onPress={handlePress}
        >
      ...
      {modalVisible && (
        <InsertItemModal onClear={() => setModalVisible(false)} />
      )}
    </View>
  );
}
